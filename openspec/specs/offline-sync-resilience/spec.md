# offline-sync-resilience Specification

## Purpose

The system supports reliable offline progress tracking and note capture by exposing an idempotent batch sync API that survives retries, partial failures, and stale cached content. The server keeps the contract honest: a re-sent event is never applied twice, a stale cached event is reported as a conflict instead of silently overwriting, and each batch item receives an explicit `applied` / `duplicate` / `rejected` / `conflict` outcome with canonical state so a mobile client can render a truthful "what still needs attention" view. The client-side bounded cache, local queue, retry policy, network transition handling, and sync status UI live in the mobile client codebase and are out of scope for this server-side specification.
## Requirements
