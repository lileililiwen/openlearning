...
The learner-facing pages deliver a consistent interaction contract across the catalog, dashboard, lesson, assessment, progress, messaging, and resource workflows. Every page defines explicit loading, empty, error, success, and disabled states, and recoverable failures provide a retry action without silently discarding safe user input. The shell preserves course context, current location, and a primary next-learning action at every depth. The same workflows remain usable on a narrow viewport without horizontal scrolling for ordinary page content, and meet WCAG 2.2 AA for semantic landmarks, labelled controls, visible focus, keyboard operation, logical focus order, and contrast.
## Requirements
