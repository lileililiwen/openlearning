...
public class ImportAsyncModel : PageModel
{
    private readonly QuizService _quizzes;
    private readonly QuestionImportService _import;

    public ImportAsyncModel(QuizService quizzes, QuestionImportService import)
    {
        _quizzes = quizzes;
        _import = import;
    }
    ...
    public async Task<IActionResult> OnPostAsync(int quizId)
    {
        ...
        Quiz = await _quizzes.GetByIdAsync(quizId);
        QuizId = quizId;
        Outcome = await _import.ImportAsync(ExcelFile, userId, quizId, Mode, isBank: false, forceAsync: true);
