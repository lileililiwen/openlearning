...
public class ExportResultsModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        var count = await _export.CountAsync(GradeExportKind.ExamAttempts, filters, userId);
        var syncMax = await _config.GetIntAsync("grade.export.syncMaxRows", 1000);
        if (count > syncMax)
        {
            var (jobId, error) = await _export.SubmitAsync(GradeExportKind.ExamAttempts, filters, userId);
        ...
        var (bytes, errorMessage, _) = await _export.ExportSyncAsync(GradeExportKind.ExamAttempts, filters, userId);
        ...
        return File(bytes, _contentTypeXlsx, $"exam-attempts-{ExamId}.xlsx");
    }
